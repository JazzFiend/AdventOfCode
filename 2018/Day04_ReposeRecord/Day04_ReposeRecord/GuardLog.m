...
- (NSInteger) computeGuardMostSleptMinuteProduct {
    NSMutableDictionary<NSString*, NSMutableArray<NSNumber*>*>* guardSleepTable = [GuardSleepCalculator populateGuardSleepTable:allNights];

    NSInteger maxSleepMinuteCounter = -1;
    NSString* guardWithMaxSleepMinute = @"";
    NSInteger maxSleptMinute = -1;
    for(NSString* guard in [guardSleepTable allKeys]) {
        for(NSInteger i = 0; i < 60; i++) {
            NSInteger currentMinuteSleepCount = [guardSleepTable[guard][i] longValue];
            if(currentMinuteSleepCount > maxSleepMinuteCounter) {
                maxSleepMinuteCounter = currentMinuteSleepCount;
                guardWithMaxSleepMinute = guard;
                maxSleptMinute = i;
            }
        }
    ...
    NSInteger guardWithMaxSleepMinuteNumber = [[guardWithMaxSleepMinute substringFromIndex:1] integerValue];


    return guardWithMaxSleepMinuteNumber * maxSleptMinute;
}
