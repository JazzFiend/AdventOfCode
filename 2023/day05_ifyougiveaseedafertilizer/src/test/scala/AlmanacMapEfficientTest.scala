...
class AlmanacMapEfficientTest extends AnyFunSpec {
  describe("Equals") {
    it("Two identical AlmanacMaps should be equal") {
      val one = AlmanacMapEfficient("source", "destination", List(MapRange(3, 2, 1)))
      val two = AlmanacMapEfficient("source", "destination", List(MapRange(3, 2, 1)))
      assert(one == two)
    }
    ...
    describe("Not equals") {
      it("Different source") {
        val one = AlmanacMapEfficient("source", "destination", List(MapRange(3, 2, 1)))
        val two = AlmanacMapEfficient("different", "destination", List(MapRange(3, 2, 1)))
        assert(one != two)
      }

      it("Different destination") {
        val one = AlmanacMapEfficient("source", "destination", List(MapRange(3, 2, 1)))
        val two = AlmanacMapEfficient("source", "different", List(MapRange(3, 2, 1)))
        assert(one != two)
      }

      it("Different mapRanges") {
        val one = AlmanacMapEfficient("source", "destination", List(MapRange(3, 2, 1)))
        val two = AlmanacMapEfficient("source", "destination", List.empty)
        assert(one != two)
      }
    }
  }
  ...
  describe("mapSourceValues") {
    ...
    it("A map that redefines a range of values with one map") {
      val almanac = AlmanacMapEfficient("a", "b", List(MapRange(10, 60, 10)))
      val expected = (0 to 59).toList.concat((10 to 19).toList).concat((70 to 100).toList)
      assert(almanac.mapSourceValues((0L to 100L).toList) == expected)
    }

    it("A map with several maps within it") {
      val almanac = AlmanacMapEfficient("a", "b", List(MapRange(90, 30, 5), MapRange(20, 80, 10)))
    ...
    it("Lots of massive maps") {
      ...
      val almanac = AlmanacMapEfficient("seed", "soil", List(
