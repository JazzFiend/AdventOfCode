class AlmanacMapEfficient(val source: String, val destination: String, val mapRanges: List[MapRange]) {
  ...
  override def equals(that: Any): Boolean = {
    that match {
      case that: AlmanacMapEfficient =>
        that.isInstanceOf[AlmanacMapEfficient] &&
        that.source == source &&
